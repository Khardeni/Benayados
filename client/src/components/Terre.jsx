const terreData = [
  ...
  },
  {
    ...
    label: 'Climat',
    value: 'Méditerranéen',
    detail: 'Étés chauds · Hivers doux',
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    label: 'Type de sol',
    value: 'Argilo-calcaire',
    detail: 'Riche en minéraux naturels',
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    label: 'Méthode',
    value: 'Mixte',
    detail: 'Pluvial & Irrigué combinés',
  },
]
    ...
    <section id="terre" className="py-24 md:py-36 terre-bg relative">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="text-center mb-16">
          <p className="section-subtitle reveal">Notre Terre</p>
          ...
          <p className="mt-5 font-body font-300 text-earth-500 max-w-xl mx-auto leading-relaxed reveal reveal-delay-2">
            La région de Tebourba bénéficie d'un micro-climat méditerranéen idéal, avec des sols riches qui nourrissent naturellement nos cultures.
          </p>
        </div>

        {/* Cards grid */}
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-16">
          {terreData.map(({ icon, label, value, detail }, i) => (
            <div
              key={label}
              className={`card-base p-7 reveal reveal-delay-${i + 1}`}
            >
              <div className="w-12 h-12 bg-olive-50 text-olive-600 flex items-center justify-center mb-5">
                {icon}
              </div>
              <p className="font-body text-xs text-earth-400 tracking-[0.2em] uppercase mb-1">{label}</p>
              <p className="font-display text-xl text-earth-800 font-600 mb-1">{value}</p>
              <p className="font-body text-sm text-earth-500 font-300">{detail}</p>
            </div>
          ))}
        </div>
        ...
        <div className="relative reveal reveal-delay-2">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <p className="font-display italic text-2xl md:text-3xl text-earth-800/80">
              "La terre de Tebourba, notre fierté."
            </p>
          </div>
        </div>
      </div>
